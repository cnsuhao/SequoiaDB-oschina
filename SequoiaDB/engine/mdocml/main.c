	...
	for ( i = 1; i < argc; ++i )
	{
		if ( argv[i][0] == '-' )
		{
			...
			case 'I':
			  ...
			  defos = mandoc_strdup(argv[i] + 5);
			  break ;
			case 'm':
				if ( ! moptions(&type, argv[i]+2))
				  return((int)MANDOCLEVEL_BADARG);
				break ;
			case 'O':
				(void)strlcat(curp.outopts, argv[i]+2, BUFSIZ);
			  (void)strlcat(curp.outopts, ",", BUFSIZ);
			  break ;
			case 'T':
				if ( ! toptions(&curp, argv[i]+2))
				  return((int)MANDOCLEVEL_BADARG);
			  break;
			case 'W':
				if ( ! woptions(&curp, argv[i]+2))
				  return((int)MANDOCLEVEL_BADARG);
				break ;
			case ('V'):
				version();
