...
for /r "%~dp0" %%f in (*.*) do (
   ...
   if "%%~nf%%~xf"=="!FULLPROGRAM!"  (
      ...
      link /OUT:!SCRIPTPATH!\build\!PROGRAM!.exe /LIBPATH:!SCRIPTPATH!\..\..\lib sdbcpp.lib "!SCRIPTPATH!\!PROGRAM!.obj" "!SCRIPTPATH!\!COMMON!.obj" /debug
      copy !SCRIPTPATH!\..\..\lib\sdbcpp.dll !SCRIPTPATH!\build
      goto :end
   )
)
